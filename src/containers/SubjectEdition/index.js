...
import { 
  ...
  addAssignatureAction, 
  barLoaderAction
} from '../../redux/studyDuck';
import AppBarMenu from '../../components/AppBarMenu';
import { updateAssignatures } from '../../api/api.js';
...
const SubjectEdition = ({ match, history }) => {
  ...
  useEffect(() => {
    if (getLists.length !== 0) {
      ...
      console.log('quizModified', quizModified)
    }
  }, [getLists])
  ...
  const onHandleAcceptModal = () => {
    onHandleCloseModal();
    const erased = [...state.quiz.filter((item, index) => index !== itemSelected)];
  ...
  const handleSaveReactive = () => {
    dispatch(barLoaderAction(true));
    updateAssignatures('quiz', match.params.url, {'quizly': state.quiz});
    dispatch(barLoaderAction(false));
  }; 
  ...
  return (
    <>
      <AppBarMenu title="Editar evaluación" icon={<KeyboardBackspaceIcon onClick={() => onHandleBack()} />} />
        <List className={classes.root}>
        {state.quiz.map((item, index) => {
          return (
            <>
              <ListItem className={classes.element} key={index} alignItems="flex-start">
              ...
              <Divider className={classes.separator} variant="inset" component="li" />
            </>
          )
        })
      }
